...
func newTestServer(t *testing.T) *server.Server {
	...
	// A policy start dials its targets, so the manager is given a fake dialer:
	// these tests are about the HTTP surface, not about reaching a device.
	dialer := &gnmi.FakeDialer{Session: &gnmi.FakeSession{Caps: &gnmi.CapabilitiesResult{}, SampleReplay: time.Hour}}
	manager := policy.NewManager(ctx, logger, policy.Options{ProfilesRoot: testProfilesRoot(t), Dialer: dialer})
	return server.NewServer("localhost", 8079, logger, manager, "1.0.0")
}
...
func namedPolicyBody(t *testing.T, name string) []byte {
	t.Helper()
...
func TestDeletePolicy_EveryAcceptedNameIsAddressable(t *testing.T) {
	...
	for _, name := range names {
		t.Run(name, func(t *testing.T) {
			...
			w := postPolicy(t, srv, "application/x-yaml", namedPolicyBody(t, name))
...
func TestDeletePolicy_RefusedNamesDoNotAddressTheirPolicy(t *testing.T) {
	...
	// collection entirely.
	base, err := url.Parse("http://127.0.0.1:8079/api/v1/policies")
	require.NoError(t, err)
	assert.Equal(t, "http://127.0.0.1:8079/api/v1", base.JoinPath(url.PathEscape("..")).String())
...
// The agent reads the capability list to decide what a policy may ask for, so
// it has to reach the wire, not only the manager.
func TestGetCapabilities(t *testing.T) {
	srv := newTestServer(t)

	w := httptest.NewRecorder()
	req, err := http.NewRequest(http.MethodGet, "/api/v1/capabilities", nil)
	require.NoError(t, err)
	srv.Router().ServeHTTP(w, req)
	...
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
	assert.Equal(t, []string{"targets", "on_change", "sample", "get"}, got.Capabilities)
}
