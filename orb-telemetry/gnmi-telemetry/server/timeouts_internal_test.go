...
// DELETE /policies/:policy waits for the policy's sweep to notice cancellation,
// which the sweep checks for between addresses, so what is left to wait for is
// the probes already in flight: one probe timeout.
func TestWriteTimeout_OutlastsTheSlowestHandler(t *testing.T) {
	srv := newInternalTestServer(t)

	const sweepCancelCeiling = config.DefaultProbeTimeoutMs * time.Millisecond
	assert.Greater(t, srv.httpServer.WriteTimeout, sweepCancelCeiling)
