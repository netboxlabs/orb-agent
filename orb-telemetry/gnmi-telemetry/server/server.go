...
// still holds roughly 2900 targets that each carry a full tls block, or 32000
// bare hosts, which is far past what one agent should subscribe to.
...
const (
	...
	// slowest is DELETE /policies/:policy, which waits for the policy's sweep
	// to notice cancellation: the sweep checks between addresses, so what is
	// left to wait for is the probes already in flight, bounded by one probe
	// timeout. POST /policies is nowhere near it, returning as soon as the
	// targets are handed to the collector. Sixty seconds keeps that ceiling
	// clear of the deadline even when the read window is spent first.
...
// Server represents the gnmi-telemetry server
type Server struct {
	router     *gin.Engine
...
// NewServer returns a new gnmi-telemetry server
...
// Start starts the gnmi-telemetry server
func (s *Server) Start() <-chan error {
	errCh := make(chan error, 1)
	go func() {
		s.logger.Info("starting gnmi-telemetry server", "address", s.httpServer.Addr)
