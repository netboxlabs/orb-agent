...
import (
	...
	"github.com/netboxlabs/orb-agent/orb-telemetry/gnmi-telemetry/version"
)
...
//
// The flush comes first, before anything has been cancelled. Every target
// loop's context derives from the root one, so cancelling it ends the loops,
// and the policy stop that follows withdraws the series they wrote. A flush
// placed after the cancellation therefore races that withdrawal for exactly the
// readings it is there to export, and a withdrawal that wins empties the final
// export for the device it belonged to. Exporting first settles the race for
// every device rather than for most of them, since the store the flush reads
// still holds them all.
//
// The cancellation then comes before the server stop, which is what lets
// srv.Stop return: stopping the server first would wait on work that has not
// been told to finish, a wait as long as the sweep probes still outstanding.
//
// The server stop comes last because every metric this backend exports is an
// observable instrument reading the collector's series store, and stopping the
...
// device data at all, which with a long export period is everything received
...
//
// Exporting before the cancellation costs the readings of a notification that
...
func main() {
	...
	port := flag.Int("port", 8079, "server port")
	...
	profilesDirFlag := flag.String("profiles-dir", "",
		"directory of metric profile YAML files overlaid on the profiles bundled into the "+
			"binary; a file replaces the bundled profile of the same name."+
			" Environment variable can be used by wrapping it in ${} (e.g. ${GNMI_PROFILES_DIR})")
	policyEnvVars := flag.String("policy-env-vars", "",
		"comma-separated environment variable names a policy may read through a ${NAME} "+
			"reference in username, password or a tls file path. Empty, the default, rejects "+
			"every reference.")
	profilesRootFlag := flag.String("profiles-root", "",
		"directory a policy's own profiles_dir must resolve inside. A policy names an absolute "+
			...
			"profiles_dir."+
			" Environment variable can be used by wrapping it in ${} (e.g. ${GNMI_PROFILES_ROOT})")
	...
	// in-flight subscriptions running after a stop signal.
	...
	profilesDir := env.ResolveEnvOrExit(*profilesDirFlag)
	manager := policy.NewManager(rootCtx, logger, policy.Options{
		DefaultProfilesDir: profilesDir,
		ProfilesRoot:       env.ResolveEnvOrExit(*profilesRootFlag),
