...
func newTestManagerRootedAt(root string) *Manager {
	// The fake dialer for the same reason newTestManager uses one: these tests
	// start policies, and a policy start dials its targets.
	return NewManager(context.Background(), testLogger, Options{ProfilesRoot: root, Dialer: testDialer()})
}
...
func TestValidateProfilesDir_RejectsAnOverrideWithNoRootConfigured(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "--profiles-root")
}
...
func TestValidateProfilesDir_RejectsWhatTheRootDoesNotContain(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, err)
			assert.Contains(t, err.Error(), "profiles directory")
		})
	}
}
...
func TestStartPolicy_RejectsProfilesDirReachedThroughASymlink(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(outside, "_base.yaml"), []byte(`
match: {}
subscriptions:
...
func TestStartPolicy_ReportsAnAbsentProfilesDirInsideTheRoot(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "profiles directory not found")
...
func TestStartPolicy_DropsTheCollectorWhenTheStartFails(t *testing.T) {
	...
	require.NoError(t, m.StartPolicy("policy-a", minimalPolicy()))
...
func TestStartPolicy_SharesOneCollectorAcrossPoliciesNamingOneDirectory(t *testing.T) {
	...
	assert.Same(t, m.policies["policy-a"].collector, m.policies["policy-b"].collector)
}
