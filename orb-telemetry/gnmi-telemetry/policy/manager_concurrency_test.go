...
func TestManagerPolicyMapIsConcurrencySafe(t *testing.T) {
	...
	policyFor := func(host string) config.Policy {
		return config.Policy{
			...
			Scope: config.Scope{
				Targets: []config.Target{{Host: host, Port: 9339}},
			},
		}
	}
...
func (g *gatedCollector) CollectTarget(_ context.Context, _ config.Target, _ collector.Options) error {
	return nil
}

func (g *gatedCollector) TargetStatuses(string) []collector.TargetStatus { return nil }
...
func TestStopPolicy_HoldsTheNameUntilTheRunnerHasStopped(t *testing.T) {
	...
	r, err := NewRunner(m.ctx, testLogger, "p1", minimalPolicy(), gate, testDialer())
	require.NoError(t, err)
	r.Start()
	m.policies["p1"] = r
	...
	go func() { started <- m.StartPolicy("p1", minimalPolicy()) }()
...
func TestSameNameStartStopIsConcurrencySafe(t *testing.T) {
	...
	for range workers {
		...
		go func() {
			defer wg.Done()
			for range rounds {
				_ = m.StartPolicy("shared", minimalPolicy())
				m.HasPolicy("shared")
...
// Only the name being stopped is held. A slow sweep shutdown must not
...
func TestStopPolicy_DoesNotBlockOtherNames(t *testing.T) {
	...
	gate := newGatedCollector()
	t.Cleanup(gate.unblock)
	r, err := NewRunner(m.ctx, testLogger, "slow", minimalPolicy(), gate, testDialer())
	require.NoError(t, err)
	r.Start()
	m.policies["slow"] = r
	...
	require.NoError(t, m.StartPolicy("other", minimalPolicy()))
...
func TestStopPolicyHandle_LeavesAReplacementAlone(t *testing.T) {
	m := newTestManager()
	h, err := m.StartPolicyHandle("p1", minimalPolicy())
	require.NoError(t, err)
