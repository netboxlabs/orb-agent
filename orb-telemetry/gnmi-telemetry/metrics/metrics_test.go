...
func TestEndpointOptions_MalformedURLRejected(t *testing.T) {
	for _, endpoint := range []string{"http://%zz", "http://", "http://:4317", "grpc:///path", "htps://collector.example:4317", "ftp://collector.example:4317", "http://collector.example:99999", "grpc://collector.example:0", "http://-collector:4317", "grpc://col_lector:4317"} {
		_, err := endpointOptions(endpoint)
		assert.Error(t, err, endpoint)
	}
	for _, endpoint := range []string{"http://c:4317", "https://c:4317", "grpc://c:4317", "grpcs://c:4317", "GRPCS://c:4317", "https://c", "grpc://[::1]:4317", "http://10.0.0.1:4317"} {
		_, err := endpointOptions(endpoint)
...
// ResetMeter is how a test unbinds the package from a previous test's
// provider, and an instrument is bound to the meter that created it. So the
// instrument caches go with the meter: a counter handed back from the cache
// after a reset would still write to the provider that is gone.
func TestResetMeterClearsInstrumentCaches(t *testing.T) {
	ResetMeter()
	t.Cleanup(ResetMeter)

	firstReader := sdkmetric.NewManualReader()
	firstProvider := sdkmetric.NewMeterProvider(sdkmetric.WithReader(firstReader))
	t.Cleanup(func() { _ = firstProvider.Shutdown(context.Background()) })
	SetMeterProviderForTest(firstProvider)
	require.NotNil(t, GetCounter("x", ""))

	ResetMeter()
	assert.Nil(t, GetCounter("x", ""), "with no meter there is no counter, cached or not")
	...
	SetMeterProviderForTest(secondProvider)
	counter := GetCounter("x", "")
...
func hasMetric(t *testing.T, reader sdkmetric.Reader, name string) bool {
	t.Helper()
	var rm metricdata.ResourceMetrics
	require.NoError(t, reader.Collect(context.Background(), &rm))
	for _, sm := range rm.ScopeMetrics {
...
// otel.metric.overflow, so the metric stops answering any question. A counter
// update carries the device and path the target named, so its label values are
// the ones a target influences, and the limit is chosen rather than inherited.
//
...
func TestPolicyMeter_ExportsOneScopePerPolicy(t *testing.T) {
	...
	ctx := context.Background()
	a, err := PolicyMeter("core").Int64Counter("gnmi.test_total")
	require.NoError(t, err)
	b, err := PolicyMeter("edge").Int64Counter("gnmi.test_total")
	require.NoError(t, err)
	a.Add(ctx, 1)
	b.Add(ctx, 2)
	...
	for _, sm := range rm.ScopeMetrics {
		assert.Equal(t, "gnmi-telemetry", sm.Scope.Name)
...
// The test hook installs a provider, so the plain meter, the health
// counters and the policy meters all come from it, and ResetMeter takes
// them away.
