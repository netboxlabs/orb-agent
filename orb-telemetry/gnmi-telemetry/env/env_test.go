...
func TestReference(t *testing.T) {
	tests := []struct {
		...
		{name: "a whole reference", value: "${GNMI_PASSWORD}", wantName: "GNMI_PASSWORD", wantOk: true},
		...
		{name: "an unclosed reference", value: "${GNMI_PASSWORD", wantOk: false},
		{name: "a reference with text around it", value: "prefix-${GNMI_PASSWORD}", wantOk: false},
		{name: "a reference with a trailing suffix", value: "${GNMI_PASSWORD}-suffix", wantOk: false},
	}
...
func TestResolveEnv_SubstitutesAReference(t *testing.T) {
	t.Setenv("GNMI_PASSWORD", "s3cret")

	got, err := ResolveEnv("${GNMI_PASSWORD}")
...
func TestResolveEnv_RejectsAnUnsetName(t *testing.T) {
	_, err := ResolveEnv("${GNMI_PASSWORD_UNSET}")
...
func TestResolveEnv_RejectsANameSetToEmpty(t *testing.T) {
	t.Setenv("GNMI_PASSWORD", "")

	_, err := ResolveEnv("${GNMI_PASSWORD}")
