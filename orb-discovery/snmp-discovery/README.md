...
Each OID is resolved independently, so a single file can cover as many models as you need and every device gets its own name under one policy. A model you add or change here also wins over the part number some vendors' chassis rows report for a standalone device (see the backend docs' Override precedence); stack members keep their own chassis models. If you run the agent in a container, mount this directory into it.
