...
func TestConnectPacketBuilder_FirstCallUsesInitialToken(t *testing.T) {
	...
	builder := buildConnectPacketBuilder(conn, "initial-token")
...
func TestConnectPacketBuilder_NilWhenNoRefresher(t *testing.T) {
	...
	builder := buildConnectPacketBuilder(conn, "some-token")
...
func TestConnectPacketBuilder_AutoReconnectRefreshesAfterFirstCall(t *testing.T) {
	// Simulates multiple auto-reconnect cycles after the initial connect.
	// First call skips refresh; all subsequent calls refresh.
	logger := slog.New(slog.NewTextHandler(os.Stderr, nil))
	conn := NewMQTTConnection(logger, noopPM{}, make(chan struct{}, 1), make(chan struct{}, 1), noopBackendState{})
	...
	builder := buildConnectPacketBuilder(conn, "managed-token")
	require.NotNil(t, builder)
...
func TestConnectPacketBuilder_EachConnectGetsOwnClosure(t *testing.T) {
	// Two separate builder instances should each have their own first-call state.
	logger := slog.New(slog.NewTextHandler(os.Stderr, nil))
	conn := NewMQTTConnection(logger, noopPM{}, make(chan struct{}, 1), make(chan struct{}, 1), noopBackendState{})
	...
	builder1 := buildConnectPacketBuilder(conn, "token-a")
	builder2 := buildConnectPacketBuilder(conn, "token-b")
