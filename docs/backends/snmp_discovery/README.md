...
- **The VLAN must already be known and named**: the association only attaches a VLAN snmp-discovery already found in the device's own VLAN data with a non-empty name. Unlike `create_unknown_vlans` (above), which stubs an unknown VID so the interface↔VLAN link is complete, this feature never invents or stubs a VLAN to satisfy an SVI name — a miss is simply left unassociated.
