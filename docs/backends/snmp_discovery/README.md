...
- **Coverage differs from device-discovery**: snmp-discovery corroborates VLANs from the Q-BRIDGE `dot1qVlanStaticTable`, plus, on Cisco devices only, the VTP VLAN table; device-discovery reads the full VLAN database over SSH via the driver's VLAN support. The same device can therefore get a VLAN association from one backend's run and not the other's.
