...
**The polled address is not claimed as the master's primary IP either**, for the same reason and through a different matcher. `primary_ip4` is unique in NetBox and outranks the name, so a master carrying a member's address resolves to that member's row and is written as the chassis master. Nothing in the walk says which member owns the address: management addresses sit on an SVI, which belongs to the stack rather than to a member, so 48 of the 52 addresses across the stacks in the reference corpus cannot be attributed at all.

This applies to stacks only, and it withholds rather than clears: Diode applies partial updates, so a primary IP already recorded in NetBox is untouched. A stack discovered for the first time gets no primary IP on its master until an operator sets one.

The master then matches on `sysName` + site or `asset_tag`, and those agree across every target of the same stack, which is what keeps re-runs upserting the existing chassis. Note this leaves two matchers rather than three, so a stack whose `sysName` does not match its NetBox name needs `asset_tag` set. Members match on `virtual_chassis` + `vc_position`, which is what identifies them; they never carried the target's `netbox_id` in any case.

**Pinning a stack's identity: use `asset_tag`, not `netbox_id`.** `defaults.asset_tag` is per policy rather than per target, so every target of the same stack stamps the same value on the master and they converge instead of diverging. It is also Diode's highest-precedence device matcher, and member Devices have it explicitly cleared so they do not collapse onto one row. It only *matches* an existing NetBox device if that device already carries the tag; otherwise the first run writes it. This is also what makes `emit_device_name: false` usable on a stack, since that option needs `source_match` or `asset_tag` to be present and `netbox_id` is no longer available here.
