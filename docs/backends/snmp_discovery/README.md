...
- **IPv6 fallback**: where the modern table reports no usable prefix, the deprecated `IPV6-MIB` `ipv6AddrPfxLength` is read if the device answers it. It is believed only when neither current table resolved a prefix, and only where its interface index agrees with the address's own binding, since it is indexed per interface and one address may appear on several. Most devices reporting IPv6 do not implement it; Junos is the common case that does. Addresses it lists that `ipAddressTable` omits (typically link-locals) are not emitted, since the table describes prefixes rather than addresses.
