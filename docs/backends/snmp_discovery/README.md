...
- **The VLAN must already be known and named**: the association only attaches a VLAN whose name the **device itself reported** in its VLAN table. A VID known only from a row status, or whose name column came back empty (or NUL padding), does not qualify — even though such a VID is still emitted as a VLAN entity under the placeholder name `VLAN<vid>`. Unlike `create_unknown_vlans` (above), which stubs an unknown VID so the interface↔VLAN link is complete, this feature never invents or stubs a VLAN to satisfy an SVI name and never attaches the placeholder — a miss is simply left unassociated.
- **Unanimity, and its limit**: a prefix is only tagged when every contributing address resolves to the same VLAN; any disagreement, or any contributing address with no resolvable VLAN, leaves the prefix untagged by design. That outcome is logged as a warning only when at least one contributing address actually proposed a VLAN — a prefix none of whose addresses sit on an SVI (every routed subnet on the device) is the ordinary case and stays silent. That check only spans the addresses seen within one device's discovery run — it does **not** protect a prefix shared across multiple devices, because a `Prefix` is a globally-keyed NetBox object and each run only sees its own target: if two devices report the same network through different VLANs, the last device to report it wins and discovery cannot correct it.
...
- **Coverage differs from device-discovery**: snmp-discovery corroborates VLANs from the Q-BRIDGE `dot1qVlanStaticTable`, plus, on Cisco devices only, the VTP VLAN table (`CISCO-VTP-MIB::vtpVlanName`, walked *only* while this option is enabled, so a Cisco target with the option off emits exactly the VLAN entities it emitted before the option existed); device-discovery reads the full VLAN database over SSH via the driver's VLAN support. The same device can therefore get a VLAN association from one backend's run and not the other's.
