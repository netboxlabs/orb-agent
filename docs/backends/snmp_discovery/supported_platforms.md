...
|-------|----------------|------------------|
| **Generic** | Q-BRIDGE-MIB (RFC 4363, `1.3.6.1.2.1.17.7.1.4`) + BRIDGE-MIB `dot1dBasePortIfIndex` | VLAN catalog (`dot1qVlanStaticTable` — names + admin status), per-port PVID (`dot1qPvid`), per-VLAN egress + untagged port lists (`dot1qVlanStatic{Egress,Untagged}Ports`, filled from `dot1qVlanCurrent{Egress,Untagged}Ports` for any VLAN the static table does not mention — a switch can run a VLAN and place ports in it untagged while listing it in neither the static table nor `dot1qPvid`. **This changes what many switches report**, not only ones that were previously blank: most devices publishing the current table publish no static egress column at all, so it becomes their sole membership source, and it can also supply an untagged row that supersedes a port's PVID or an egress row that clears a routed inference), read as the bitmap the MIB defines, or, on Junos only, as the comma-separated bridge port numbers it publishes by default when the values could not be bitmaps. Required for trunk classification. A port is access when its one VLAN is the one it is untagged in, or the one its PVID names when the device publishes no untagged row for that VLAN; any VLAN a port is only tagged in makes it a trunk, one tagged VLAN included, and a PVID of 0 is read as no untagged VLAN rather than as VLAN 0. |
...
| **Huawei catalog** | HUAWEI-VLAN-MIB `hwVlanMIBTable` (`1.3.6.1.4.1.2011.5.6.1.1`): `hwVlanIndex`, `hwVlanName`, `hwVlanRowStatus` | The VLAN catalog on Huawei platforms that implement no Q-BRIDGE-MIB at all, notably SmartAX OLTs (MA5600T / MA5608T), which answer "No Such Object" for `dot1qVlanStaticTable` and `dot1qPortVlanTable`. Supplies VLAN IDs, names and status only; a VLAN with no configured description has no `hwVlanName` row and is emitted under the `VLAN<vid>` placeholder. Walked only when `sysObjectID` falls under enterprise prefix `1.3.6.1.4.1.2011.`. Where a Huawei switch also answers Q-BRIDGE, the standard column's name wins. |
...
| Huawei SmartAX OLT (MA5600T / MA5608T) | ❌ Not implemented (`dot1qVlanStaticTable`, `dot1qPortVlanTable` and `dot1dBasePortIfIndex` all answer "No Such Object") | ✅ Huawei catalog | VLAN entities with IDs, names and status; no interface `mode` / `tagged_vlans` / `untagged_vlan`, since the vendor port list is keyed by bridge port and the device publishes nothing to translate it with |
