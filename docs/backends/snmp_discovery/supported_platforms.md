...
**Optics without a serial.** A module bay requires a serial on the module installed in it, so an optic reporting none is discovered but never emitted — its would-be bay and model are dropped along with it. Skipped optics are aggregated into a single warning per device rather than one line per optic, so a platform publishing dozens of serial-less rows doesn't flood the log every poll.
