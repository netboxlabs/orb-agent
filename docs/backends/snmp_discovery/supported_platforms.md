...
**A PVID alone is not always configuration.** RFC 4363 gives `dot1qPvid` a DEFVAL of 1, so a bridge with VLAN filtering disabled still answers 1 for every port because the agent must return something. Where a device publishes no VLAN of its own — no static names or row statuses, no membership masks, no VTP catalog — a port reporting that default is left unclassified and no VLAN is created for it, since there is nothing to corroborate the value against and reading it as an access assignment invents a VLAN the operator never configured. A non-default PVID is still configuration and classifies as before, as does any PVID once the device names a VLAN somewhere. Measured on MikroTik RouterOS 6 and 7, which answer the bridge-port and port-VLAN tables while returning `No Such Object` for the whole Q-BRIDGE VLAN subtree.
