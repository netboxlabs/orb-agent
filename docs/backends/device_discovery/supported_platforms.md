...
| `nokia_sros` | Supported (Nokia SR OS via NETCONF) — 7750 SR-7 / SR-12 / SR-12e, 7750 SR-7s / SR-14s, and 7950 XRS-20 modular chassis. Issues two subtree filters against the `urn:nokia.com:sros:ns:yang:sr:state` model: the first walks `state/card` (with nested `mda` list) to build the chassis → card → MDA tree, the second walks `state/port/transceiver` so transceivers nest under their owning MDA. IOM / IMM cards classify as `linecard`, CPM-A/B as `supervisor`, switch-fabric modules (SFM) as `linecard`. Fixed-form-factor SR-1 / SR-1s emit no module entries. |
| `nokia_sros_ssh` | Supported (Nokia SR OS via SSH CLI) — same hardware coverage as `nokia_sros`. Driver-local regex parsers consume `show chassis detail`, `show card detail`, `show mda detail`, and per-port `show port X/Y/Z detail` (no ntc-templates exist for the inventory commands). Output envelope is byte-identical to the NETCONF path for the same chassis — operators can switch transports without re-discovering modules. |
...
**Nokia SR OS (`nokia_sros`, NETCONF).** Walks the chassis → card → MDA → transceiver tree at depth 3 (the maximum the canonical envelope supports). The card pass and the transceiver pass are issued as two separate ncclient subtree-filtered `<get>` RPCs; the transceiver pass is best-effort — if it fails, a card-only envelope still ships. CPM-A / CPM-B slot ids surface as letter strings (e.g. `A`, `B`) and are emitted verbatim. Switch-fabric modules (SFM) are classified as `linecard` because NetBox's Module schema does not carry a distinct fabric concept today (matches `eos` and `nxos` behaviour). Top-level bays are sorted with letter slots first (CPMs) then numeric slots ascending, so the rendered NetBox view is stable regardless of device-side ordering.

**Nokia SR OS (`nokia_sros_ssh`, SSH CLI).** Functional equivalent of `nokia_sros` for deployments where NETCONF is unavailable or disabled. Parses `show chassis detail`, `show card detail`, `show mda detail`, and per-port `show port X/Y/Z detail` via driver-local regex over the SR-OS `===`-separated block format (ntc-templates does not provide `alcatel_sros_*` templates for the inventory commands). Header and field rows straddle a `===` separator in each block, so the parser tracks pending state across two consecutive blocks. The envelope is byte-identical to the NETCONF path for the same physical chassis; this parity is asserted in tests by comparing the SSH and NETCONF expected results against the same JSON fixture.
