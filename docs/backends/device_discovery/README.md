...
| device_name_source | string | Fact used for `Device.name`: `hostname` (default) or `fqdn`. With `fqdn`, falls back to the hostname when the driver reported no usable FQDN — a missing/empty value, the placeholders `None` (junos) / `Unknown` (ios family), or a value equal to the hostname (no domain configured). Note: Diode matches devices by name, so switching an existing deployment to `fqdn` creates new records unless the NetBox devices are renamed first. Virtual-chassis member names built from `stack_member_name_template` keep using the raw hostname fact. |
